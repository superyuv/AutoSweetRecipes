...
class PageCreateRecipe extends StatefulWidget {
  const PageCreateRecipe({Key? key}) : super(key: key);
...
class _PageCreateRecipeState extends State<PageCreateRecipe> {
                  ...
                  Padding(padding: const EdgeInsets.fromLTRB(3, 0, 3, 0),
                      child: SizedBox(
                        ...
                        child: FloatingActionButton(
                          ...
                          onPressed: () async {
                            if (inventoryController.text.trim() != "" && !ingredients.contains(inventoryController.text.trim().toLowerCase()))
                            {
  ...
  Future <bool> checkData() async {
    var exist = await firebaseService.readRecipeByName(recipeNameController.text);
    if (exist.docs.isNotEmpty){
      _illegalData(context, "Name already exist");
      return false;
    }

    if (recipeNameController.text == '') {
      _illegalData(context, "Missing recipe name");
      return false;
    }
    if (ingredients == []) {
